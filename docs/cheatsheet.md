...
```bash
# Type checking
npm run typecheck          # backend + frontend (combined)
npm run typecheck:backend  # backend only (src/* ยกเว้น frontend)
npm run typecheck:frontend # frontend only (src/frontend/**/* React/TSX)

# Backend only
npm run dev -- 10           # ts-node dev mode (10s interval)
npm run dev:backend -- 10  # alias for backend only

# Frontend only (Vite dev server with proxy to backend)
...
# Production build
npm run build         # backend (esbuild) + frontend (vite)
...
| `src/frontend/routes/*.tsx` | 11 page routes |
| `src/frontend/components/ui/*.tsx` | UI primitives (Button, Card, Skeleton, Avatar, Badge, etc.) |
| `src/frontend/components/DataTable.tsx` | Reusable data table with column sorting |
| `src/frontend/components/Sparkline.tsx` | SVG sparkline mini charts |
| `src/frontend/components/EmptyState.tsx` | Polished empty state component |
| `src/frontend/components/layout/AppLayout.tsx` | Sidebar (Cmd+K search) + top bar + mobile bottom tabs |
...
> - Settings save → live reload ทันที (DB → process.env sync) — ไม่ต้อง restart
> - `notify-rules.json` ไม่ safe สำหรับ multi-instance writes
> - Dashboard (`HTTP_ENABLED=true`) ต้องการ MySQL สำหรับ auth และข้อมูล
