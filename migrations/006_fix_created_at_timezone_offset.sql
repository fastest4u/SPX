SET @sql = IF(
  ...
  'UPDATE spx_booking_history SET created_at = DATE_SUB(created_at, INTERVAL 7 HOUR)',
  'SELECT 1'
);
...
SET @sql = IF(
  ...
  'UPDATE users SET created_at = DATE_SUB(created_at, INTERVAL 7 HOUR)',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = IF(
  ...
  'UPDATE audit_logs SET created_at = DATE_SUB(created_at, INTERVAL 7 HOUR)',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = IF(
  ...
  'UPDATE metrics_snapshots SET created_at = DATE_SUB(created_at, INTERVAL 7 HOUR)',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
