---
name: spx-awaken
description: Assess current SPX project state and recommend next actions from Memory Vault evidence. Use when the user invokes `$spx-awaken`, asks to awaken, asks what to do next, or wants the Codex equivalent of `/awaken`.
---
...
2. `memory/00_Index/Awakened-AI-System.md` — Current coverage and open gaps.
3. `memory/00_Index/Multi-AI-Acceptance-Results.md` — Agent acceptance status.
4. `memory/00_Index/Open-Followups.md` — What tasks are still open across sessions?
5. `memory/00_Index/Session-Threads.md` — What storylines are in progress?
6. `memory/00_Index/Vault-Dashboard.md` — What's the vault health telling us?

### 2. Load Recent Tactical Context

Read:

7. The **last 3-5 session logs** from `memory/05_Agent_Session_Logs/` (sorted by date DESC).
8. `memory/04_Architecture_Decisions/` — All ADRs. Note `status: accepted` decisions and their "Consequences / Follow-ups" sections.
9. `memory/07_Insights/` — What patterns have been promoted? Are there gaps?
10. `memory/08_Mistakes/` — Open mistakes that need prevention.

### 3. Load Code State (if applicable)

If the user is asking about code development (not pure memory/docs):

11. `src/` — Skim `app.ts`, key controllers, and services for TODOs or incomplete features.
12. `package.json` scripts — What commands exist but might need enhancement?
...
Run `npm run memory:verify` before stating memory health.

### 6. Synthesize Recommendations

Output to the user in this format (Thai when user writes Thai):
