---
name: spx-self-check
description: Run SPX evidence, risk, and verification checks before risky work. Use when the user invokes `$spx-self-check`, asks for self-check, or before production-impacting changes involving src, DB schema, auth, secrets, deploy, Docker, notifications, auto-accept, or runtime settings.
---
...
7. **Verification plan** — pick the right gate:
   - Memory-only: `npm run memory:verify`
   - Code only: targeted typecheck/build gate from root `AGENTS.md`
   - Code + memory: `npm run verify`
   - DB drift: `npm run schema:verify` only when DB env is intentionally available.

8. **For production-impacting commit/push/review/merge work**, read `.windsurf/workflows/strict-pr-review-8-category.md` before proceeding.

9. **Output a self-report** in this format:
...
10. **Then proceed** with the task — but with eyes open.
...
**Past mistakes to avoid:**
- [[Mistake-003-Baseline-Migration-Drift]] — DDL can drift from source contract.
