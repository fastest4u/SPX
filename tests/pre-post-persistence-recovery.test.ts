...
function fixture(path: Path) {
  ...
  db.prepare("INSERT INTO notify_rules (id,team_id,name,origins,destinations,vehicle_types,need,auto_accept,accept_all) VALUES (?,1,?,'[\"NORC-B\"]','[\"SOCW\"]','[\"6WH\"]',?,1,?)")
    .run(rule.id, rule.name, rule.need, acceptAll ? 1 : 0);
  let posts = 0;
  const list = (pending: boolean): BookingRequestListResponse => ({ retcode: 0, message: "", data: {
    pageno: 1, count: 100, total: pending ? 1 : 0,
    request_list: pending ? [{ request_id: requestId, booking_id: bookingId,
      booking_date: 1781136000, standby_time: 960, cost_type: 1, trip_type: 1, shift_type: 0,
      vehicle_type: 13, vehicle_type_name: "6WH", request_acceptance_status: 1, request_assignment_status: 0,
  ...
  const client = {
    fetchBookingRequestList: async (_id: number, options: { tabPendingConfirmation?: boolean } = {}) =>
      posts > 0 && options.tabPendingConfirmation === false ? null : list(options.tabPendingConfirmation !== false),
  ...
  return { bookingId, booking, rule, db, harness, runner, client, jobs, posts: () => posts,
