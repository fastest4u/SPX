...
function showModal(id) { const el = document.getElementById(id); if (el) { el.style.display = "flex"; el.classList.add("modal-open"); } }
function hideModal(id) { const el = document.getElementById(id); if (el) { el.style.display = "none"; el.classList.remove("modal-open"); } }

let pendingDeleteRuleIds = [];
let pendingDeleteRuleId = null;
let pendingDeleteRuleConfirmMessage = "";
...
function getColumnIndex(key, columnKey) { return TABLE_SCHEMAS[key].columns.findIndex((c) => c.key === columnKey); }
function isVisibleColumn(key, columnKey) { return columnVisibility[key][columnKey] !== false; }
...
function requestDeleteRule(id) { pendingDeleteRuleId = id; showModal("confirmDeleteModal"); }
async function confirmDeleteRule() { if (!pendingDeleteRuleId && pendingDeleteRuleIds.length === 0) return; try { if (pendingDeleteRuleIds.length > 0) { for (const id of pendingDeleteRuleIds) { const res = await fetch(`${API}/rules/${encodeURIComponent(id)}`, { method: "DELETE" }); if (!res.ok) throw new Error("Failed"); } showToast(`ลบ ${pendingDeleteRuleIds.length} รายการแล้ว`); } else { const res = await fetch(`${API}/rules/${encodeURIComponent(pendingDeleteRuleId)}`, { method: "DELETE" }); if (!res.ok) throw new Error("Failed"); showToast("ลบรายการแล้ว"); } fetchGrid("rules"); reloadAudit(); } catch { showToast("เกิดข้อผิดพลาด", true); } finally { pendingDeleteRuleId = null; pendingDeleteRuleIds = []; pendingDeleteRuleConfirmMessage = ""; document.getElementById("confirm-delete-rule-button").textContent = "ลบรายการ"; document.querySelector("#confirmDeleteModal .modal-body p").textContent = "ต้องการลบรายการค้นหานี้หรือไม่?"; hideModal("confirmDeleteModal"); } }
async function addUser() { const username = document.getElementById("u-username").value.trim(); const password = document.getElementById("u-password").value.trim(); const role = document.getElementById("u-role").value; if (!username || !password) return showToast("กรุณากรอกข้อมูลให้ครบถ้วน", true); if (password.length < MIN_PASSWORD_LENGTH) return showToast(`รหัสผ่านต้องยาวอย่างน้อย ${MIN_PASSWORD_LENGTH} ตัวอักษร`, true); try { const res = await fetch(`${API}/users`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, password, role }) }); if (!res.ok) throw new Error("Failed"); document.querySelectorAll("#addUserModal input").forEach((input) => { input.value = ""; }); hideModal("addUserModal"); showToast("เพิ่มผู้ใช้งานสำเร็จ"); fetchGrid("users"); } catch { showToast("เกิดข้อผิดพลาด หรือชื่อผู้ใช้ซ้ำ", true); } }
function openChangePassword(id) { document.getElementById("pwd-user-id").value = id; document.getElementById("pwd-new").value = ""; showModal("changePasswordModal"); }
async function changePassword() { const id = document.getElementById("pwd-user-id").value; const password = document.getElementById("pwd-new").value.trim(); if (password.length < MIN_PASSWORD_LENGTH) return showToast(`รหัสผ่านต้องยาวอย่างน้อย ${MIN_PASSWORD_LENGTH} ตัวอักษร`, true); try { const res = await fetch(`${API}/users/${encodeURIComponent(id)}/password`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password }) }); if (!res.ok) throw new Error("Failed"); hideModal("changePasswordModal"); showToast("เปลี่ยนรหัสผ่านสำเร็จ"); fetchGrid("users"); } catch { showToast("เกิดข้อผิดพลาด", true); } }
...
function openAcceptConfirm(bookingId, requestId) { pendingAcceptBookingId = Number(bookingId); pendingAcceptRequestIds = [Number(requestId)]; document.getElementById("accept-preview").textContent = JSON.stringify({ bookingId: pendingAcceptBookingId, requestIds: pendingAcceptRequestIds }, null, 2); showModal("confirmAcceptModal"); }
async function confirmAcceptJob() { if (!pendingAcceptBookingId || !pendingAcceptRequestIds) return; try { const res = await fetch(`${API}/bidding/accept`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bookingId: pendingAcceptBookingId, requestIds: pendingAcceptRequestIds, confirm: true }) }); const result = await res.json(); if (res.ok && result.ok) { showToast("รับงานสำเร็จ"); reloadHistory(); reloadAudit(); } else { showToast(result?.error?.message || "รับงานไม่สำเร็จ", true); } } catch { showToast("เกิดข้อผิดพลาดในการรับงาน", true); } finally { pendingAcceptBookingId = null; pendingAcceptRequestIds = null; hideModal("confirmAcceptModal"); } }
