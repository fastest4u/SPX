...
function buildHeaders(): Record<string, string> {
  ...
  // 2026-06-11: the `user-agent` header is REQUIRED, not optional. PROVEN by an
  // A/B test against the live SPX endpoint with a valid DB cookie:
  //   no user-agent          → HTTP 403, empty body (SGW gateway block)
  //   + user-agent (Chrome)  → HTTP 200, retcode 0, full list
  //   + origin only (no UA)  → HTTP 403
  // The SGW gateway rejects any request without a user-agent BEFORE it reaches
  // SPX (so the failure surfaces as a 403 the error-classifier mislabels as
  // session_expired). A brief 2026-06-11 revert that dropped these two headers
  // took production fully blind until this was restored. The earlier
  // "lost every bid after #52" was the slow POLL_INTERVAL_MS, NOT these
  // headers — do not remove them again.
  return {
